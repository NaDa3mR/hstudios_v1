                ...
                <div class="taskboardapp-wrap">
                    <div class="taskboardapp-content">
                        <div class="taskboardapp-detail-wrap">
                            ...
                            {{-- Topbar --}}
                            @include('clients-dashboard.service-request.topbar')
                            ...
                            @endif
                                ...
                                <div class="card mx-4 mx-md-5 shadow-5-strong bg-body-tertiary"
                                    ...
                                    <div class="card-body py-5 px-md-5">
                                        ...
                                        <h2 class="fw-bold mb-4">My Requests</h2>

                                        @forelse($requests as $request)
                                        <div class="card mb-4 p-3 text-start">
                                            <h4 class="fw-bold">{{ $request->name ?? 'Unnamed Request' }}</h4>
                                            <p><strong>Status:</strong> {{ $request->status ?? 'Pending' }}</p>
                                            <p><strong>Details:</strong> {{ $request->details ?? 'No details provided.' }}</p>
                                            <p><strong>Services:</strong></p>
                                            <ul>
                                                @forelse($request->services as $service)
                                                    <li>{{ $service->name }}</li>
                                                @empty
                                                    <li>No services linked.</li>
                                                @endforelse
                                            </ul>
                                            <p><strong>File Request:</strong></p>
                                            <div class="d-flex align-items-center">
                                                <div class="mt-2">
                                                    @if ($request->hasMedia('service_file'))
                                                        <a href="{{ $request->getFirstMediaUrl('service_file') }}"
                                                            target="_blank"
                                                            class="btn  btn-sm">
                                                            View Request File
                                                        </a>
