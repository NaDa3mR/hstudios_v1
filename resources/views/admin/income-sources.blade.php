...
<body>
                ...
                <div class="taskboardapp-wrap">
                    <div class="taskboardapp-content">
                        <div class="taskboardapp-detail-wrap">
                            @include('admin.sections.income-sources.topbar')
                            @section('blog-header-action')
                            @endsection

                            @include('admin.sections.income-sources.table')
                        </div>

                        @include('admin.sections.income-sources.add_modal')
                        @foreach ($income_sources as $income_source)
                            @include('admin.sections.income-sources.update_modal')
                            @include('admin.sections.income-sources.delete_modal')
                        @endforeach
                    </div>
                </div>
            </div>
    ...
    <script>
        $('.toggle-status').on('change', function() {
            ...
            $.ajax({
                url: "{{ route('in-source.toggleStatus') }}",
                method: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    id: id,
